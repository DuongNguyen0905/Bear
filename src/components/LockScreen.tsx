...
const LockScreen: React.FC<{ onUnlock: () => void }> = ({ onUnlock }) => {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'var(--bg-gradient), var(--bg-main)', zIndex: 9999, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '24px' }}>
      <div style={{ width: '84px', height: '84px', borderRadius: '50%', background: 'var(--glass-bg-strong)', border: '1px solid var(--border-glass)', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '24px', cursor: 'pointer' }} onClick={attemptBiometric}>
        <Fingerprint size={40} color="var(--primary)" />
      </div>
      <h2 style={{ margin: '0 0 6px 0' }}>Sổ tay của tớ</h2>
      <p style={{ margin: '0 0 28px 0', color: 'var(--text-muted)', fontSize: '14px', textAlign: 'center' }}>
        {tryingBiometric ? 'Chạm biểu cảm vân tay để mở...' : 'Nhập mã PIN 6 số để mở'}
      </p>
